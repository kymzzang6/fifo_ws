...
class IntegratedPPENode(Node):
    def __init__(self):
        ...
        # (웹캠용이던 camera_id, frame_width, frame_height, timer_period 파라미터는 삭제됨)
        ...
        # ========== CvBridge 및 Subscriber 생성 (핵심 변경 사항) ==========
        self.bridge = CvBridge()
        
        # Astra 카메라의 컬러 영상 토픽을 구독합니다.
        ...
        self.get_logger().info('📡 Astra 카메라 영상 구독 시작 (/camera/color/image_raw)')
    ...
    # ===========================================================
    #  이미지 콜백 (토픽으로 프레임이 들어올 때마다 자동 실행됨)
    # ===========================================================
    def image_callback(self, msg):
        try:
            # ROS 2 Image 메시지를 OpenCV 포맷(BGR8)으로 변환
        ...
        # ★ (cx, cy, x1, y1, x2, y2) — 전체 bbox 좌표도 함께 저장
        ...
        for r in ppe_results:
            for box in r.boxes:
                ...
                # ★ 전체 bbox 좌표 (cx, cy, x1, y1, x2, y2) 로 저장
        ...
        if is_human_detected:
            ...
            def make_hand_box(center, ratio, up_ratio=self.RATIO_HAND_UP):
                """
                손 전용 비대칭 박스:
                - 좌우/아래는 ratio 기준 정사각형
                - 위쪽은 up_ratio 만큼 추가 확장
                """
                if center is None:
                    return empty.copy()
                s      = (body_width * ratio) / 2.0
        ...
        def boxes_intersect(body_box, det_box):
            """
            ★ 핵심 수정: 중심점 포함 여부 → bbox 교차 여부로 변경
            두 박스가 조금이라도 겹치면 True 반환
            """
